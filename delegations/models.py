...
class Delegation(Model):
    ...
    def get_available_referees(match, referee_role):
        ...
        available_referees = qualified_referees.exclude(id__in=unavailable_referees)

        # Filtering and returning referees who are qualified, available and not already delegated at the same day
        delegated_referees = Delegation.objects.filter(
            match__date_time__date=match.date_time.date()
        ).values_list('referee', flat=True)  # Just IDs list
        return available_referees.exclude(id__in=delegated_referees)
